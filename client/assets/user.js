...
const displayAllBooks = (data) => {
  const allData = data.map((d) => {
    return `
    <div id='bookList' class="cursor-pointer flex flex-col justify-center object-cover items-center" data-id=${d.book_id}>
      <img class="object-cover" src=${d.book_image} alt="book cover"/>
      <h2 class="font-bold text-center">${d.title}</h2>
      <p>${d.author}</p>
    </div>`;
  });
...
const displayModal = (data) => {
  ...
  <div class="modal-content flex flex-col  bg-white mx-auto p-5 border-2 border-slate-500 max-w-2xl relative">
    ...
    data.book_id
  } class="py-2 px-6 bg-florin-red-light hover:bg-florin-red active:bg-florin-red-dark text-white transition-all">${
